...
@app.route('/successpay/<id1>/<name>/<price>/<q>',methods=['GET','POST'])
def success_pay(id1,name,price,q):
    today=date.today()
    day=today.day
    month=today.month
    year=today.year
    ...
    cursor=mysql.connection.cursor()
    cursor.execute('insert into orders(number,name,q,price,date) values(%s,%s,%s,%s,%s)',[session.get('name'),name,q,price,date_today])
    mysql.connection.commit()
    cursor.close()
...
def orders():
    cursor=mysql.connection.cursor()
    sql_select_query = """select * from orders where number = %s"""
    cursor.execute(sql_select_query, (session.get('name'),))
    data=cursor.fetchall()
    return render_template('orders.html',orders=data,id1=session.get('name'))

@app.route('/pay/<name>/<q>/<price>',methods=['GET','POST'])
def pay(name,q,price):
    checkout_session=stripe.checkout.Session.create(
        success_url=request.host_url+url_for('success_pay',id1=session.get('name'),name=name,q=q,price=int(q)*price),
        line_items=[
            {
                'price_data': {
                    'product_data': {
